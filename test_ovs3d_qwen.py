...
def reasoning_grouding_by_qwen(model, tokenizer, image_path, prompt):
    ...
    img_box = tokenizer.draw_bbox_on_latest_picture(response)
    box = extract_box(response, image.width, image.height)
    return img_box, box, image
...
if __name__ == '__main__':
    ...
    # use cuda device
    qwen = AutoModelForCausalLM.from_pretrained("ckpts/Qwen-VL",
                                                device_map="cuda:1",
